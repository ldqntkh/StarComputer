...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
    ...
    function woocommerce_catalog_ordering() {
        ...
        $catalog_orderby_options = apply_filters(
            ...
            getCatalogOrderByOptionsConfig()
        );
...
function getCatalogOrderByOptionsConfig() {
    $optionsConfigStr = get_option( 'custom_preferences_options' ) ? get_option( 'custom_preferences_options' )[ 'list_catalog_orderby' ] : '';
    $options = array();
    $optionsConfig = !empty( $optionsConfigStr ) ? json_decode( $optionsConfigStr, true ) : array();
    for ( $i = 0; $i < count( $optionsConfig ); $i++ ) {
        $optionID = $optionsConfig[$i]['id'];
        $optionText = $optionsConfig[$i]['text'];
        $options[$optionID] = __( $optionText, 'woocommerce' );
    }
    return $options;
}
