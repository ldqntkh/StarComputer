            ...
            if ( !isset($wp->query_vars['order-received']) || $wp->query_vars['order-received'] == '' ) {
                if ( isset($_REQUEST['appid']) && isset($_REQUEST['checksum']) && isset($_REQUEST['apptransid']) && isset($_REQUEST['status']) ) {
                    ...
                    if ($order_id) {
                        ...
                        if ($order->get_payment_method() != 'gearvn_zalo_payment') {
                            $order = null;
                        } else {
                    ...
                    if ($order->get_payment_method() != 'gearvn_momo_payment') {
                        $order = null;
                    }
                }
            } 
    ...
    <?php if ( $order ) :
        ...
        wc_get_template( 'checkout/thankyou.php', array( 'order' => $order ) ); 
            ...
            <div class="text-center w-100">
                <h4><?php echo __('Đơn hàng không tồn tại', 'gearvn'); ?></h4>
                <a href="/"><?php echo __('Quay về trang chủ', 'gearvn'); ?></a>
            </div>
            <?php
    endif;?>
</div>