...
<div class="custom-checkout">
    ...
    <div class="checkout-address">
                ...
                <div class="address-item <?php $isAddressDefault ? 'active' : ''; ?>">
                    <div class="address-content">
                        ...
                        <div class="info">
                            ...
                            <p><strong><?php echo __('Địa chỉ:' , 'gearvn') ?></strong> <span><?php echo isset($address['full_address']) ? esc_html($address['full_address']) : ''; ?></span></p>
                            <p><strong><?php echo __('Số điện thoại:' , 'gearvn') ?></strong> <span><?php echo esc_html($address['billing_phone']); ?></span></p>
                        </div>
                        <div class="group-button">
                            <span class="deliver-address" data-key="<?php echo $idx; ?>"><?php echo __('Giao đến địa chỉ này', 'gearvn'); ?></span>
                            <span class="update-address" data-key="<?php echo $idx; ?>" data-value="<?php echo esc_html(json_encode($address)); ?>"><?php echo __('Sửa', 'gearvn'); ?></span>
                            <?php if ( $user->ID !== 0 && !$isAddressDefault ): ?>
                                <span data-key="<?php echo $idx; ?>" class="remove-address" ><?php echo __('Xoá', 'gearvn'); ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <?php if ( !empty( $otherAddr ) && $user->ID !== 0 && count($otherAddr) < 10): ?>
            <p><?php echo __('Bạn muốn giao đến địa chỉ mới?', 'gearvn'); ?> <a href="#" class="add-new-address"><?php echo __('Tạo địa chỉ mới.', 'gearvn'); ?></a></p>
        <?php endif; ?>
        ...
        <div class="address-form new-address <?php  
            ...
            <form method="post" onsubmit="return false;">
                <?php if ($user->ID !== 0): ?>
                    <h2 class="title-block" data-text-add="<?php echo __('Tạo địa chỉ mới', 'gearvn'); ?>" data-text-edit="<?php echo __('Sửa địa chỉ', 'gearvn'); ?>"></h2>
                <?php endif; ?>
                <div id="addresses">
                    <div class="shipping_address address_block">
                        ...
                        <div class="input-form d-none <?php if ($user->ID === 0) echo 'hidden';?>">
                            <div class="gearvn-input-checkbox">
                                <input type="checkbox" name="address_is_default" class="choose_default_address"id="default_address" />
                                <label for="default_address" class="d-none"><?php echo __('Sử dụng làm địa chỉ mặc định', 'gearvn'); ?></label>
                            </div>
                ...
                <div class="group-button">
                    <input type="hidden" name="shipping_account_address_action" value="save" />
                    <button data-text-add="<?php echo __('Giao đến địa chỉ này', 'gearvn'); ?>" data-text-edit="<?php echo __('Cập nhật', 'gearvn'); ?>" class="save-new-address" type="button">
                        <?php echo __('Giao đến địa chỉ này', 'gearvn'); ?>
                    </button>
                    <?php if ($user->ID !== 0 && !empty( $otherAddr )):?>
                        <a href="#" class="cancel"><?php echo __('Hủy', 'gearvn'); ?></a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
...
<div class="modal-delete-address-wrapper">
    ...
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-body">
            ...
            <span class="text-center d-block"><?php echo __('Bạn có chắc chắn muốn xoá địa chỉ?', 'gearvn'); ?></span>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">
                <i class="fas fa-sign-out-alt"></i>
                <span><?php echo __('Huỷ', 'gearvn'); ?></span>
            </button>
            <a data-href="<?php echo get_permalink( wc_get_page_id( 'checkout' ) ); ?>" class="btn btn-primary btn-delete-address">
                <i class="far fa-trash-alt"></i>
                <?php echo __('Xác nhận', 'gearvn'); ?>
            </a>
        </div>
        </div>
    </div>
